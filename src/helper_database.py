...
class SociBotDB:
    ...
    def get_db_client(self):
        client = MongoClient(f"mongodb+srv://socibot:{db_password}@cluster0.mzer5.mongodb.net/?retryWrites=true&w=majority")
        db = client['socibot']
        return db
        logger.debug(db)
    ...
    def update_response_chatroom(self,response,chatroom_id):
        ...
        self.chat_room_collection.update_one({"_id": chatroom_id}, {"$set": result}, upsert=True)
    ...
    def insert_chat_room_conversations(self,chatroom_id,message,user_id):
        ...
        if not result:
            conversation_data = {
                ...
                "conversations": [
                    ...
                    },
                    {
                        "user": "bot",
                        "timestamp": datetime.datetime.utcnow().isoformat(),
                        "message": "SentiBot: Hey there! How can I help you today?"
            ...
            self.chat_room_collection.update_one({"_id": chatroom_id}, {"$set": conversation_data}, upsert=True)
            logger.debug('chatrooms collection updated!')
        else:
            result = self.chat_room_collection.find_one({"_id": chatroom_id})
            conversations = result['conversations']
    ...
    def insert_session_users(self,user_id,chatroom_id):
        ...
        if result:
            ...
            self.user_collection.update_one({"_id": user_id}, {"$push": {"chatroom_ids": chatroom_id}})
        else:
            ...
            self.user_collection.update_one({"_id": user_id}, {"$set": user_data}, upsert=True)
            logger.debug(f'User ID is not present in USERS collection, hence inserted the chatroom Info!')
